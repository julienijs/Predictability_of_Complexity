setwd("C:/Users/u0149275/OneDrive - KU Leuven/Complexity/DTA_1800-1999")
...
# read data
morph_data <- read_xlsx("zipped_morphology_DTA.xlsx", col_names = TRUE)
zipped <- read_xlsx("DTA_Zipped_Sizes.xlsx", col_names = TRUE)
...
# read data
synt_data <- read_xlsx("zipped_syntax_DTA.xlsx", col_names = TRUE)
...
morph_and_synt <- morph_and_synt[morph_and_synt$year >= 1863 & morph_and_synt$year <= 1927, ]
...
morph_ts <- ts(morph_and_synt$morph, start = 1863, frequency = 1)
...
synt_ts <- ts(morph_and_synt$synt, start = 1863, frequency = 1)
...
grangertest(synt_diff_ts ~ morph_diff_ts, order = 2)
grangertest(morph_diff_ts ~ synt_diff_ts, order = 2)
...
tsDF <- data.frame(Year = seq(from = 1863, to = 1927, by = 1),
                   "Syntax" = synt_ts,
                   "Morphology" = morph_ts)
...
years <- seq(1863, 1927, by = 5)  # Starting from 1863, incrementing by 5 until 1927
last_year <- 1927
years <- c(years, last_year)  # Include 1927
